    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                results, maps, times, rocaucs = test.test(
                    data_dict,
            ...
            with open(results_file, "a") as f:
                # ADD METRIC, update here
                f.write(s + "%10.4g" * 13 % (results + rocaucs) + "\n")  # append metrics, val_loss
            ...
            fi = fitness_roc(np.array(rocaucs).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            if fi > best_fitness:
                best_fitness = fi
                if wandb_logger.wandb:
                    wandb_logger.log({"best_fitness": best_fitness})  # W&B
                    wandb_logger.log({"best_fitness_epoch": epoch})  # W&B
...
if __name__ == "__main__":
    ...
    parser.add_argument("--initial_lr", type=float, default=0.0, help="Initial learning rate")
